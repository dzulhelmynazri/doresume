...
5. Career sites without an adapter: escalate to the Browserbase browser tools. Use `browserbase__create_session`, then `browserbase__navigate` to the application page, `browserbase__observe` to discover form fields, `browserbase__act` to fill each field and upload the resume and cover letter, and `browserbase__extract` to capture the confirmation. Prefer `browserbase__fetch` when a page only needs reading; start a browser session only when the page requires interaction.
6. Always call `browserbase__stop_session` when the browser task finishes, whether it succeeded or failed, so billable browser time is released.
